...
data class OffsetTargetTemperatureRequest
@JvmOverloads
constructor(
  /** The seats to apply the offset to. Must be non-empty. */
  val seats: Set<String>,
  /** The temperature offset. Must be positive. */
  val offset: Temperature,
  /**
   ...
   * value before setting if needed. If {@code false}, the exact temperature will try to be set, but
   * it may return error if the value is not supported for the action.
   */
  val roundToNearestSupportedValue: Boolean = true,
) {
  init {
    require(seats.isNotEmpty()) { "Seats must be non-empty" }
    require(offset.value > 0) { "Offset must be positive" }
  }
}
